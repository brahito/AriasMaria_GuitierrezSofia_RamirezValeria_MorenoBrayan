...
public abstract class Arana extends Thread {
	...
	public void run() {
		try {
			while (vivo) {
				if (congelado == false) {
					if (validarPerseguirSerpiente(mundo.getSerA()) == true && fresas > mundo.getSerA().getN()) {
						perseguirSerpiente(mundo.getSerA());
					} else if (validarPerseguirSerpiente(mundo.getSerA()) == true && fresas < mundo.getSerA().getN()) {
						huirSerpiente(mundo.getSerA());
					} else {
				...
				if (validarSerpiente(mundo.getSerA()) && fresas > mundo.getSerA().getN() && mundo.getSerA().getN() >= 1) {
					mundo.getSerA().quitarCola();
					fresas++;
				}
				if (validarSerpiente(mundo.getSerA()) && fresas > mundo.getSerA().getN() && mundo.getSerA().getN() >= 1
						&& dientes == true) {
					mundo.getSerA().quitarColaTodas();
					fresas += mundo.getSerA().getN();
				}
				sleep(50);
			}
